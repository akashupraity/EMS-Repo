...
export default class NewRequestForm extends React.Component<INewRequestFormProps, INewRequestFormState> {
 ...
 // save all expenses into "ExpenseDetails" list in batches
  ...
  updateUniqueID = (requestorUniqueID, itemId, submissionType: string) => {
    ...
    // let updatePostStatus={
    //   Status:"Draft"
    // }
  if(!updatePostDate.RequestorID){
    let updatePostID = {
      RequestorID: this.GetUniqueRequestorID(itemId),
    }
    this._spOps.UpdateItem(this.props.expenseListTitle, updatePostID, itemId)
    .then((response) => {
      if (this.state.IExpenseModel.length > 0) {
    ...
    // this._spOps.UpdateItem(this.props.expenseListTitle, updatePostStatus, itemId).then(() => {
    //   alert("Your request is not submitted please re-submit");
    //       this.RefreshPage();
    // })

  }else{
  //if Requestor ID is not a blank so run this code
  this._spOps.UpdateItem(this.props.expenseListTitle, updatePostDate, itemId)
  .then((response) => {
    if (this.state.IExpenseModel.length > 0) {
      this.AddExpensesDetailsAsBatch(this.state.IExpenseModel, itemId).then(() => {
        this.setState({
  ...
  //     if (response === "Updated") {
  //       if (this.state.IExpenseModel.length > 0) {
  //         this.AddExpensesDetailsAsBatch(this.state.IExpenseModel, itemId)
  //           .then(() => {
  //             this.setState({ loading: false });
  //             $('#loader').hide();
  //             alert(submissionType == "Submitted" ? "Request submitted successfully" : "Request drafted successfully");
  //             this.RefreshPage();
  //           });
  //       } else {
  //         this.setState({ loading: false });
  ...
  //       }
  //     } else {
  //       // If the response is not "Updated", show an error alert
  //       this.setState({ loading: false });
  //       $('#loader').hide();
  //       alert("Your request was not created. Please resubmit.");
  //       this._spOps.UpdateItem(this.props.expenseListTitle, updatePostStatus, itemId)
  //     }
  //   })
  //   .catch((error) => {
  //     // Handle any errors from the update function
  //     this.setState({ loading: false });
  //     $('#loader').hide();
      ...
      return (<div key={idx}>
        <div className={styles.renderExpenseTbl}>
          <div className="form-group col-md-2">
            ...
            <select className='form-control ExpTypeSelectOptions' 
            name="Expense" value={this.state.IExpenseModel[idx].Expense} 
          ...
          {item.Expense == "Hotel" &&
            <div className="form-group col-md-2">
              <label className="control-label">Check Out &nbsp;<span 
              className={styles.noteMsg}>(MM-DD-YYYY)</span> <span className={styles.star}>*</span></label>
              <input
          ...
          {item.Expense == "Travel" &&
            <div className="form-group col-md-1">
              ...
              <input
                ...
                type="number"
                min="1"
                disabled={(item.TravelType == "Leased/Self Owned" || !this.state.IExpenseModel[idx].StartMile||item.TravelType == "Cab/Uber/Lyft"|| item.TravelType == "Airfare") ? true : false}
                className={styles.formControl} 
                name="EndMileout"
